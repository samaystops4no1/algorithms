...
//
//  Created by samay jain on 28/05/17.
//  Copyright © 2017 samay jain. All rights reserved.
//
...
void search(string text,string pattern,int q){
   ...
   for(int i=0;i<patternLength;i++){
        textHash=(textHash+text[i])%q;
        patternHash=((patternHash)+pattern[i])%q;
    ...
    int initial=0;
    int i;
    for(i=patternLength;i<textLength+1;i++){
        if(textHash==patternHash){
           //now I will match every single character
            int z=0;
            int j;
            for(j=initial;j<(initial+patternLength)-1;j++){
                if(pattern[z]==text[j]){
                    z++;
                }
                else{
            ...
            if(j==(initial+patternLength)-1){
                cout<<"Word completely matched at index "<<initial+1<<endl;
                
            }
            else{
               cout<<"Hash matched but word did not match"<<endl;
            }
        
        }
        
        
            textHash=(((textHash-(text[initial])))+text[i])%q;
            initial++;
